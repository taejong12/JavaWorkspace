...
public class CommentsDAO extends JDBConnPool{
	...
	public int insertComm(CommentsDTO dto) {
		...
		try {
			//지금 작성하는 댓글 단계가 자식단계일 때는 현재 작성하는 칸 바로 밑에 위치시켜 주기 위해,
			if(comm_num!=0) {
				psmt = con.prepareStatement(sql2);
				psmt.setInt(1, dto.getRegroup()); //그룹번호에 있는
				psmt.setInt(2, dto.getRegorder()); //부모의 모든 댓글들을 불러서
				psmt.executeUpdate();
				psmt.close();
				dto.setRegorder(dto.getRegorder()+1); // 정렬 단계를 하나 증가시킨다.
				dto.setReglevel(dto.getReglevel()+1); // 단계에 따라 간격을 더 늘려주기 위한 레벨숫자도 1 늘려준다.
			}
			//현재 DB에 있는 번호보다 1 증가시켜가져와서 comm_num을 부여하기 위함. 
			...
			//부모단계일 때의 그룹번호에, 위에서 1 증가되어 부여된 현재 본인의 comm_num을 셋팅
			if(comm_num==0) {
	...
	public int deleteComm(int comm_num) {
		...
		try {
			...
			String query = "UPDATE COMMENTS SET ME_NUM = NULL, WRITER = NULL, COMM_REGDATE = NULL, COMM_CONTENT = '삭제된 댓글입니다.', PASS = NULL "
					+" WHERE COMM_NUM=?";
			psmt = con.prepareStatement(query);
